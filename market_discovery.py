...
def _sonnet_entry_analysis(opportunity):
    ...
    if not allowed:
        return _sonnet_failure_result(reason)
    ...
    try:
        ...
        return _sonnet_failure_result("anthropic_not_installed")
    ...
    try:
        ...
        prompt = (
            "Analyze this weather market entry candidate.\n"
            f"City: {city}\n"
            f"Date: {date}\n"
            ...
            "Return a single-line JSON object only with keys: confidence, recommendation, sonnet_temp_c, metar_temp_c, nws_forecast_c, reasoning. "
            "Do not include markdown, code fences, or extra text. recommendation must be enter or skip."
        )
        ...
        return _sonnet_failure_result(f"sonnet_error_fallback:{str(exc)[:80]}")
